@extends('admin.layouts.app')
    ...
    <div class="card mb-5">
       ...
       <div class="card-body">
        <form action="{{ route('cost.search') }}" method="get" role="search">
                    ...
                    <tbody>
                        @foreach ($data as $i=>$order)
                            <tr>
                                <td>{{ ++$i }}</td>
                                <td>{{ $order->orderID }}</td>
                                <td>{{ $order->StakeholderName }}</td>
                                <td>{{ number_format($order->orderAmount,2) }}</td>
                                <td>{{ $order->comission }} %</td>
                                <td>{{ number_format($order->given_amount,2) }}</td>
                                <td>{{ date('m-d-Y', strtotime($order->pay_date)) }}</td>                         
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
